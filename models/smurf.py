...
class SMURF(TensorBase):
    def __init__(self, aabb, gridSize, device, **kargs):
        super(SMURF, self).__init__(aabb, gridSize, device, **kargs)
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [{'params': self.density_line, 'lr': lr_init_spatialxyz}, {'params': self.density_plane, 'lr': lr_init_spatialxyz},
                     ...
                     {'params': self.kernelnet.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        if isinstance(self.tonemapping, torch.nn.Module):
            grad_vars += [{'params':self.tonemapping.parameters(), 'lr':lr_init_network}]
        return grad_vars
    ...
    def upsample_volume_grid(self, res_target):
        ...
        print_log(f'Upsamping to {res_target}')
    ...
    def shrink(self, new_aabb):
        ...
        newSize = b_r - t_l
        self.aabb = new_aabb
        self.update_stepSize((newSize[0].item(), newSize[1].item(), newSize[2].item()))